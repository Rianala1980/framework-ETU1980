package etu1980.framework.servlet;
import etu1980.framework.Mapping;
import etu1980.framework.ModelView;
import etu1980.framework.annotation.*;
                ...
                if((boolean) s.getAttribute(sessionName)==true)
                {
                    try {
                        if (met.getAnnotation(Authentification.class).profil().compareTo((String) session.getAttribute(sessionProfil)) == 0) {
                            ...
                            if(modelView.isJson()){String json = gson.toJson(modelView.getData());out.println(json);}
